...
So ~85 GFLOP/s is the identity kernel's real throughput, not a stack-invariant ceiling. The best-known stack is `OptLocal²+OptTile+OptUpcast²+OptVec4Load` (typed `array<vec4<f32>>` bindings with 128-bit distributed tile loads) at ~4.4-5.0x identity. Three honest residuals:

- **`OptVectorize` (b37) is a measured regression** versus plain register blocking, and the mechanism is now understood: `applyVectorize` splits the TS-wide N-local axis into an outer `AxisLocal` of size `TS/W` plus an `AxisVectorize` inner of size `W`, and the lowerer derives `workgroup_size` from `AxisLocal` ranges only, so the workgroup shrinks 256→64 threads (occupancy collapse across the smem-barrier-bounded k-loop) while the "vec4" still lowers to four scalar FMAs on Metal. BEAM is benchmark-driven and never selects it (b3 is strictly faster, so b37 can never become `best`); no explicit prefilter is needed. The geometry is locked by `TestB37_GeometryRegression_WorkgroupShrink` (render-time, GPU-free) so a future change that keeps `OptVectorize` while restoring 256 threads must update the test deliberately.
- **Plain WGSL subgroups (`subgroupBroadcast`/`shuffle`/reductions) work on the gogpu/naga Metal stack** (verified, 32-wide simdgroups) but are **not a useful matmul lever at the current 16-wide tile geometry**: the workgroup is 256 threads = 8 simdgroups of 32, so only the row-shared A operand (`rA`, contiguous lanes within one simdgroup) can be subgroup-broadcast, while the higher-reuse B operand (`rB`, column-shared) is strided across all simdgroups and must stay in shared memory. No barrier is removed (the cooperative tile fills keep both smem arrays and both barriers load-bearing), so the lever is at best a redundant-smem-read elision worth <5% on a bandwidth/occupancy-bound kernel - not pursued. See `notes/subgroup_preflight.md`.
- **WGSL subgroup-matrix ops remain unavailable** on the gogpu/naga stack (no parser/MSL support) and are not yet in the WebGPU spec, so tensor-core-class throughput stays out of reach.

### Separate `Realize` calls do not share scheduling or compilation work

`Realize` is stateless: each call schedules the graph from its leaves (rangeify, bufferize, kernel split, memory plan) and renders plus compiles every kernel. The schedule cache (`arenaSchedCache` in `schedule/cache.go`) is keyed by the sink node index, and every `Realize` builds a fresh `OpSink` (`tensor/realize.go`), so realizing several outputs of one shared graph in separate calls re-schedules and recompiles the shared subgraph each time. A training step that realizes the loss and each gradient in its own call therefore does N+1 full schedule-and-compile passes over the same forward and backward.

The production workaround is to batch: realize the loss and all gradients in one `Realize` call, so the shared graph is scheduled, compiled, and executed once. Every example training loop does this. Measured impact on the heavy ResNet9 backward: a per-grad loop ran over 5 minutes per step; the batched call runs in about 55 seconds.

A partial, opt-in mitigation exists. Stateful realize (`ANNEAL_STATEFUL_REALIZE=1`, or `tensor.SetStatefulRealize`) caches intermediate buffers across `Realize` calls within a scope (keyed by arena `RealizeID` and `RealizeGen`) and skips their producer kernels, so separate calls skip re-execution; it is verified bit-identical to stateless realize and invalidates on any leaf data change. It does NOT skip re-scheduling or re-compilation, which dominate for large graphs (per-grad ResNet9 with the buffer cache is about 143 seconds, still well above batching's 55 seconds), so it is off by default and batching stays the recommended path.

Roadmap: making separate `Realize` calls also skip scheduling and compilation requires the scheduler to become incremental and cross-sink, rangeifying each node once per arena and caching per-kernel schedules plus rendered WGSL by cross-sink structural identity rather than by sink index. That is a rewrite of the scheduler, the most complex subsystem, where kernel boundaries, buffer-slot assignment, and execution order are currently global decisions over a single sink. It is deferred deliberately: batching already achieves one schedule, one compile, and one execution per step, so the rewrite would buy ergonomics (the natural per-grad loop matching batched speed) rather than a new capability. Full analysis in `notes/stateful_realize_finding.md`.
