...
func TestShapeTrackerBasic(t *testing.T) {
	...
	if !shapesEqual(st.Shape(), []int64{4, 5}) {
...
func TestShapeTrackerReshapePush(t *testing.T) {
	...
	st := base.Pad([][2]int64{i2(1, 1)})
	// Now reshape to (2,4): active view is non-contiguous. Reshape can't reuse strides
	// (mask crosses boundaries for non-aligned sizes) → should push new view.
	st2 := st.Reshape([]int64{8})
	// shape=(8,) reshape to (8,): same shape, no push.
...
func TestShapeTrackerViewStack(t *testing.T) {
	...
	st = st.Pad([][2]int64{i2(1, 1)}) // shape=(8,), non-contiguous
	// Reshape to (4,2): mask (1,7) on shape (8,) → new_shape (4,2).
	// mask=(1,7): loOuter=1//2=0, hiOuterIncl=(7-1)//2=2, cross-boundary → fail → push new view.
	...
	if !shapesEqual(st2.Shape(), []int64{4, 2}) {
...
func TestShapeTrackerMovementChain(t *testing.T) {
	...
	st = st.Shrink([][2]int64{i2(1, 5), i2(0, 4)}) // (4,4)
	...
	if v.Offset != 19 {
		t.Errorf("chain offset=%d want 19", v.Offset)
	}
...
func TestShapeTrackerExpandReshape(t *testing.T) {
	...
	v := st.ActiveView()
	if !shapesEqual(v.Shape, []int64{12}) {
		t.Fatalf("shape=%v want [12]", v.Shape)
	}
	if !v.Contiguous {
		t.Error("pushed view should be contiguous")
	}
...
func TestShapeTrackerImmutable(t *testing.T) {
	...
	if shapesEqual(st1.Shape(), st2.Shape()) {
		t.Error("permute should change shape")
	}
	if !shapesEqual(st1.Shape(), []int64{3, 4}) {
