...
3. **Dtype breadth — runtime lowering. RESOLVED.**
   ...
   - **fp8** (e4m3fn + e5m2, OCP FP8) on the bf16 decoded-storage scheme: the u32 storage word holds the fp8-quantized value's full f32 bit pattern, loads are a free `bitcast<f32>`, and RTNE narrowing happens at the store boundary (`_fp8e4m3_rtne_bits` / `_fp8e5m2_rtne_bits`, mirroring `uop.Float32ToFP8E4M3/E5M2` bit for bit). e4m3fn uses CUDA satfinite overflow semantics (finite overflow and ±Inf saturate to ±448); e5m2 overflow rounds to ±Inf. Any adapter, no extension. See LIMITATIONS.md §fp8 for carried scope limits.
   - **Tolerances:** elementwise atol=1e-3 (f16 ε ≈ 9.77e-4); small matmul atol=1e-2 rtol=1e-3 (TVM `test_to_mixed_precision`); chained atol=5e-2 rtol=1e-2 (tinygrad PR #7973: rtol=atol=2e-3 violated after 7 convs); bf16 FD atol=rtol=0.3 (7-bit mantissa quantization noise/h analysis); fp8 GPU-vs-host bit-exact (max-abs-diff=0; the host oracle and WGSL store helpers share one algorithm).
   - **Deferred:** FP16/FP16 accumulation (fast-path opt-in); explicit mixed-precision training with loss scaling (Parameter.Value as f32 master gives implicit mixed precision; named API is deferred); fp8 packed storage and scaled-matmul recipes (amax tracking).
