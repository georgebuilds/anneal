...
// Theme is owned by the host site (docs/index.html): the page-level toggle
// writes localStorage["anneal-theme"] and sets [data-theme] on <html> before
// first paint. The renderer's stylesheet honours that attribute, so this file
// no longer manages theme state.
...
function setStats(stageStats) {
  ...
  parts.push('(frozen trace from the real compiler)');
  el.textContent = parts.join('  ·  ');
}

// ── Data loading ──────────────────────────────────────────────────────────
// Static page: fetch the frozen JSON trace co-located beside this script.

async function loadTimeline() {
  const resp = await fetch('trace.json', { cache: 'force-cache' });
  if (!resp.ok) throw new Error('trace.json ' + resp.status);
  const data = await resp.json();
  if (data.error) throw new Error('trace: ' + data.error);
  return data;
}
...
// stage so the seq filter composes with stage Overrides.
...
function applyStage(stageIdx) {
  ...
  // nodes and there's at least one fired rule to scrub through.
...
async function loadAndRender() {
  setStatus('loading frozen trace…');
  ...
  try {
    ...
    setStatus(`${data.name} · frozen trace · ${data.stages.length} stages · arrows to scrub`);
  } catch (e) {
